...
engine_cms = create_engine(SQLALCHEMY_DATABASE_URI_CMS,pool_size=20, max_overflow=20)
...
if __name__ == '__main__':
    ...
    result = session_cms.query(SwInfo).filter(
        and_(SwInfo.infoisdisplay == '1', SwInfo.columnid.in_((2, 3)))).order_by(
        desc(SwInfo.infodatetime)).offset(0).limit(10).all()
    # result = query.all()
    for v in result:
        print v.infocontent
    pass